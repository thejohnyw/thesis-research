"""
Train/evaluate Baseline 2 (text-only):
- Frozen BERT embeddings from pre-game NBA news text
...
from baseline2 import backtest_baseline2, build_text_only_matrix
from baseline3 import build_text_embeddings_from_articles_jsonl, build_text_embeddings_from_games
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Baseline 2 (text-only BERT embeddings)")
    ...
    parser.add_argument("--text_only_csv", type=str, default="data/processed/text_only_features.csv")
    parser.add_argument("--results_csv", type=str, default="data/processed/baseline2_results.csv")
    ...
    parser.add_argument("--model_type", choices=["logistic", "mlp"], default="logistic")
    ...
    text_only_path = project_root / args.text_only_csv.replace(".csv", f"{suffix}.csv")
    ...
    text_only_df = build_text_only_matrix(features_df=features_df, text_embeddings_df=text_embeddings_df)
    text_only_path.parent.mkdir(parents=True, exist_ok=True)
    text_only_df.to_csv(text_only_path, index=False)
    print(f"Saved text-only matrix -> {text_only_path}")
    print(f"Text-only shape: {text_only_df.shape}")

    text_only_with_outcomes = text_only_df[~text_only_df["home_win"].isna()].copy()
    if args.allow_missing_text:
        args.require_text = False
    if args.require_text and "text_article_count" in text_only_with_outcomes.columns:
        before = len(text_only_with_outcomes)
        text_only_with_outcomes = text_only_with_outcomes[text_only_with_outcomes["text_article_count"] > 0].copy()
        print(f"Filtered to games with matched text: {len(text_only_with_outcomes)}/{before}")

    min_rows = 20 if args.first_x > 0 else 100
    if len(text_only_with_outcomes) < min_rows:
        raise ValueError(f"Too few outcome rows for training: {len(text_only_with_outcomes)} (min {min_rows})")
    ...
    min_train_games = max(5, len(text_only_with_outcomes) // 4) if args.first_x > 0 else 100
    print("Running time-based backtest for Baseline 2")
    results_df = backtest_baseline2(
        text_only_with_outcomes,
